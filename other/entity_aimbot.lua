...
local function getBestTarget()
	...
	for _, v in ipairs(ents.GetAll()) do
		if v:IsPlayer() or v:IsWorld() or v:IsDormant() or not stuff.aiments[v:GetClass()] then -- Don't waste time on bad entities
			continue
		end
...
hook.Add("CreateMove", "@", function(cmd)
	...
	if stuff.meth.aimbot.shooting then -- Aimbot entities
		...
		if not shouldAimbot() or not IsValid(targ) then -- If stopped aimbotting or target goes away, stop
